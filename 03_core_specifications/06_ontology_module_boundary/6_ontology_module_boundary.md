...
본 문서는 온톨로지 중심 사이버-물리 플랫폼에서 사용되는 Ontology Module Boundary의 핵심 규칙을 정의한다.

Ontology Module Boundary는 전체 온톨로지를 하나의 거대한 파일이나 하나의 거대한 클래스 집합으로 만들지 않고, 의미적으로 책임이 다른 모듈들을 분리하는 기준이다.

Event Type이 “무슨 일이 발생했는가”를 표현하고, Action Type이 “어떤 대응을 할 수 있는가”를 표현하며, State Model이 “현재 무엇이 어떤 상태인가”를 표현하고, Evidence Model이 “그 판단을 무엇으로 증명할 수 있는가”를 표현한다면, Ontology Module Boundary는 “어떤 의미 개념이 어느 온톨로지 모듈에 속해야 하는가”를 표현한다.

본 문서는 다음에 집중한다.

Ontology Module이 무엇인지 정의한다.  
Core Ontology와 Domain Ontology의 경계를 구분한다.  
Core를 core-upper와 core-crosscutting으로 분리한다.  
Construction, Industrial, Robot, Policy, AI, Evidence, Event, State, Action, Mapping 모듈의 책임을 정의한다.  
이전 Evidence Model 문서와 Evidence Ontology Module의 연결점을 명시한다.  
공통 개념과 도메인 개념을 구분한다.  
모듈 간 의존성 방향을 DAG 구조로 정의한다.  
Object Property의 domain / range를 통한 우회 오염을 방지한다.  
복잡한 cross-domain 관계를 Mediation Concept로 중재한다.  
Runtime 환경에서 OWL Reasoning과 SHACL Validation의 역할을 분리한다.  
External Mapping Module을 Semantic Adapter로 격리한다.  
Mapping Module의 독립 versioning, hot-swap, rollback, conflict resolution 정책을 정의한다.  
Namespace와 IRI 설계 기준을 정의한다.  
Versioned IRI와 owl:imports 예시를 제공한다.  
Module ownership, SemVer versioning, deprecation rule을 정의한다.  
Impact Level 기반 governance를 정의한다.  
CI/CD 기반 module boundary validator를 정의한다.  
Ontology drift, domain pollution, vendor pollution, AI pollution, property pollution을 방지한다.  
MVP에서 먼저 구현할 ontology module set을 정의한다.

전체 class/property catalog는 별도 문서인 Appendix E: Ontology Module Catalog에서 관리한다.

---

# **2\. 문서 분리 원칙**

Ontology Module Boundary 문서는 두 부분으로 나눈다.
...
Ontology Module 정의  
Layered Semantic Architecture  
Core Upper와 Core Crosscutting 구분  
Core Ontology와 Domain Ontology의 구분  
Common / Core Module 책임  
Construction Module 책임  
Industrial Module 책임  
Robot Module 책임  
Policy Module 책임  
AI Module 책임  
Evidence Module 책임  
Event / State / Action Module 책임  
External Mapping / Semantic Adapter 책임  
모듈 간 의존성 규칙  
Object Property Boundary Rule  
Mediation Concept Pattern  
Reasoning Wall 방지 규칙  
Runtime SHACL Validation Rule  
Namespace / IRI 규칙  
owl:imports 규칙  
Import / Reference Rule  
...
이렇게 분리하면 Boundary 문서는 짧고 안정적으로 유지되고, 세부 class/property catalog는 도메인 확장에 따라 계속 확장할 수 있다.

---

# **3\. Ontology Module의 정의**

Ontology Module은 특정 의미 영역을 책임지는 온톨로지 단위다.

하나의 플랫폼은 하나의 거대한 온톨로지 파일로 운영하면 안 된다.

대신 다음처럼 의미 책임을 나누어야 한다.
...
Ontology Module은 다음 질문에 답해야 한다.

이 모듈은 어떤 의미 영역을 책임지는가?  
이 모듈 안에 어떤 class가 들어갈 수 있는가?  
이 모듈 안에 어떤 property가 들어갈 수 있는가?  
이 모듈의 ObjectProperty는 어떤 domain / range를 가질 수 있는가?  
이 모듈이 import할 수 있는 다른 모듈은 무엇인가?  
이 모듈이 참조할 수 있는 다른 모듈은 무엇인가?  
이 모듈이 참조하면 안 되는 모듈은 무엇인가?  
복잡한 cross-domain 관계는 어떤 mediation concept를 통해 연결되는가?  
이 모듈의 owner는 누구인가?  
이 모듈의 versioning 정책은 무엇인가?  
이 모듈의 변경 impact level은 무엇인가?  
이 모듈이 외부 표준 온톨로지와 어떻게 연결되는가?  
이 모듈의 변경이 runtime system에 어떤 영향을 주는가?  
이 모듈은 offline reasoner 검증 대상인가, runtime SHACL 검증 대상인가?

---

# **4\. Layered Semantic Architecture**

Ontology Module Boundary는 단방향 비순환 그래프, 즉 DAG 구조를 가져야 한다.
...
Core는 Domain을 알아서는 안 된다.  
Domain은 Core를 확장한다.  
Mapping은 외부 스키마와 내부 의미 체계를 연결한다.  
Registry는 운영 규칙을 관리한다.  
Application Layer는 Ontology를 직접 오염시키지 않고 Registry와 Adapter를 통해 접근한다.

---

# **5\. 핵심 철학**

Ontology Module Boundary의 핵심 철학은 다음이다.

가장 안정적인 공통 개념은 core-upper에 둔다.  
도메인 간 공유되지만 확장 가능성이 큰 개념은 core-crosscutting에 둔다.  
도메인 특화 개념은 Domain Module에 둔다.  
외부 표준과 vendor schema는 Mapping Module에 둔다.  
runtime object는 DTO / World State / Registry에 둔다.  
의미 정의는 Ontology에 둔다.  
물리 실행 명령은 Ontology에 두지 않는다.

온톨로지는 모든 것을 담는 데이터베이스가 아니다.
...
무엇이 어떤 종류인가  
무엇이 무엇과 관계가 있는가  
어떤 상태와 행동이 의미적으로 허용되는가  
어떤 판단이 어떤 evidence를 요구하는가  
어떤 action이 어떤 target에 적용될 수 있는가  
어떤 module이 어떤 의미를 책임지는가  
어떤 cross-domain 관계가 어떤 mediation concept를 통해 연결되는가

온톨로지는 다음을 직접 수행하지 않는다.
...
# **6\. Core Ontology의 2단 구조**

기존 Core Module에 Event, State, Action, Evidence, Policy, Provenance까지 모두 넣으면 Core가 지나치게 커질 위험이 있다.
...
core-upper는 가장 안정적이고 모든 도메인에서 공통으로 쓰이는 최상위 개념을 정의한다.

core-upper는 거의 바뀌지 않아야 한다.
...
가장 안정적인 의미 기반 제공  
Domain Module의 최상위 추상 class 제공  
cross-domain 결합을 완화하는 기반 제공  
IRI 안정성 유지
...
core-crosscutting은 여러 domain에서 공유되지만 core-upper보다 확장 가능성이 높은 개념을 정의한다.
...
core-crosscutting의 역할:

Event / State / Action / Evidence / Policy 계열의 공통 추상 제공  
Domain Module 간 간접 연결 제공  
Registry와 Ontology 사이의 의미 bridge 제공  
Mediation Concept 제공

---

## **6.3 Core 분리 원칙**

core-upper는 가장 안정적인 상위 개념만 가진다.  
core-crosscutting은 도메인 간 공유되는 운영 의미 개념을 가진다.  
Domain Module은 core-upper와 core-crosscutting을 참조할 수 있다.  
core-upper는 core-crosscutting을 import하지 않는다.  
core-crosscutting은 core-upper를 import할 수 있다.

---

# **7\. Core Ontology와 Domain Ontology의 구분**

## **7.1 Core Ontology**

Core Ontology는 모든 도메인에서 공통으로 쓰이는 가장 기본적인 의미 구조다.

Core Ontology는 플랫폼의 의미 헌법이다.

Core Ontology는 자주 바뀌면 안 된다.  
Core Ontology는 최대한 작고 안정적이어야 한다.

---

## **7.2 Domain Ontology**

Domain Ontology는 특정 산업, 현장, 장비, 업무 영역에 특화된 의미 구조다.
...
Domain Ontology는 Core Ontology를 확장한다.
...
Common / Core Module은 모든 도메인에서 공유되는 최상위 개념과 crosscutting 개념을 정의한다.

이 모듈은 플랫폼 전체 의미 구조의 기준점이다.
...
Core Module에는 도메인 특화 class를 넣지 않는다.
...
이런 개념은 Construction, Industrial, Robot, Mapping 등 Domain Module에 둔다.
...
Construction Module은 건설 현장, 작업, 공간, 자재, 인력, 허가, 점검, 안전 구역과 관련된 의미를 정의한다.
...
Construction Module은 로봇의 내부 mission state나 PLC register를 직접 정의하지 않는다.
...
이런 개념은 Robot, Industrial, AI, Mapping Module에 둔다.
...
Industrial Module은 센서, 장비, PLC, SCADA, alarm, telemetry, equipment state와 관련된 의미를 정의한다.
...
Industrial Module은 작업 허가서, 공종, 로봇 미션, LLM 요약 등을 직접 정의하지 않는다.
...
Robot Module은 로봇, humanoid, AMR, AGV, drone, mission, capability, fleet manager, robot middleware와 관련된 의미를 정의한다.
...
Robot Module은 작업 허가 정책, 센서 원시 telemetry 저장 구조, AI prompt, evidence trust policy를 직접 정의하지 않는다.
...
Policy Module은 역할, 권한, 승인, 접근 제어, 안전 정책, 법적 제약, emergency policy와 관련된 의미를 정의한다.
...
Policy Module은 실제 Rego code, OPA bundle, OpenFGA tuple 자체를 온톨로지 class로 직접 관리하지 않는다.

온톨로지는 정책 의미를 정의한다.  
OPA / Rego / OpenFGA는 runtime policy execution을 담당한다.
...
AI Module은 LLM output, RAG result, model explanation, prompt trace, mapping proposal, risk interpretation, action candidate와 관련된 의미를 정의한다.
...
AI Module은 ApprovedAction, EmergencyApprovedAction, ExecutionRequest, PhysicalCommand를 직접 생성하는 의미를 가져서는 안 된다.
...
Evidence Module은 evidence type, evidence source, trust level, provenance, evidence bundle, evidence graph 관계와 관련된 의미를 정의한다.

이 모듈은 이전 문서인 Evidence Model에서 정의한 구조를 의미 계층에서 수용해야 한다.

Evidence Model은 DTO와 운영 규칙을 정의한다.  
Evidence Ontology Module은 해당 개념들의 의미 구조를 정의한다.

---

## **14.2 Evidence Model과의 명시적 연결**

Evidence Module은 Evidence Model에서 정의한 다음 개념을 의미적으로 표현할 수 있어야 한다.
...
즉, Evidence Model의 다음 필드들은 Evidence Ontology Module에서 class 또는 property로 연결될 수 있어야 한다.
...
## **14.5 Attested Extraction 연결**

Evidence Module은 AI/RAG/OCR 추출값을 직접 primary evidence로 취급하지 않는다.
...
## **14.6 Conflict Resolution Matrix 연결**

Evidence Module은 Evidence Conflict를 의미적으로 표현해야 한다.
...
Conflict resolution의 실제 계산과 가중치 적용은 Registry / Policy Layer에서 수행한다.  
Ontology Module은 그 의미 관계를 정의한다.

---

## **14.7 Privacy Lifecycle 연결**

Evidence Module은 append-only audit shell과 PII payload 보호를 구분해야 한다.
...
Evidence Module은 sensor raw stream을 직접 저장하지 않는다.

고주기 raw telemetry는 Time-Series DB 또는 Stream Layer가 담당한다.  
Evidence Module은 그 데이터가 판단 근거로 승격된 경우의 의미를 정의한다.
...
Event Module은 platform에서 사용되는 semantic event의 의미를 정의한다.
...
Event Type Registry와 연결된다.

---

## **15.2 State Module**

State Module은 entity가 가질 수 있는 semantic state의 의미를 정의한다.
...
State Model Registry와 연결된다.

---

## **15.3 Action Module**

Action Module은 platform에서 사용되는 action type의 의미를 정의한다.
...
Action Type Registry와 연결된다.

---

## **15.4 의미 정의와 운영 규칙의 경계**

Event / State / Action Module은 의미를 정의한다.

Event Type Registry, State Model Registry, Action Type Registry는 운영 가능한 registry를 관리한다.
...
External Mapping Module은 외부 표준, 외부 시스템, vendor-specific schema와 내부 ontology module 사이의 mapping을 정의한다.
...
외부 표준을 core ontology 안에 직접 섞지 않는다.

외부 표준은 mapping layer를 통해 연결한다.

---

## **16.4 Semantic Adapter Rule**

Mapping Module은 단순 보조 파일이 아니라 Semantic Adapter로 취급한다.
...
Mapping Module은 내부 ontology module과 별도 versioning track을 가져야 한다.
...
Vendor API가 변경될 경우 내부 Robot Ontology를 수정하는 것이 아니라 mapping만 교체한다.

예:

mapping:VendorRobotAPI\_v2\_to\_robot\_RobotMission

내부 개념인 `robot:RobotMission`은 안정적으로 유지한다.

---

## **16.6 Mapping Conflict Resolution Rule**

동일한 내부 class에 여러 external mapping이 붙을 수 있다.
...
이 경우 mapping conflict resolution 정책이 필요하다.
...
Mapping Module은 hot-swap과 rollback을 지원해야 한다.
...
Ontology Module은 무작위로 서로 참조하면 안 된다.
...
Domain Module은 Core Module을 참조할 수 있다.

Core Module은 Domain Module을 참조하면 안 된다.
...
모든 module dependency는 DAG 구조를 가져야 한다.
...
Cycle이 발견되면 module release를 차단해야 한다.
...
모듈 오염은 class 상속에서만 발생하지 않는다.

실제 구현에서는 ObjectProperty의 `rdfs:domain`과 `rdfs:range`를 통해 우회 오염이 발생할 수 있다.
...
겉으로 보면 class hierarchy는 깨끗하다.

하지만 property range에서 Robot Module이 Construction Module을 직접 참조하게 된다.

이러면 Robot Module과 Construction Module이 강하게 결합된다.

---

## **18.2 핵심 규칙**

Domain Module에 속한 ObjectProperty는 타 Domain Module의 class를 `rdfs:domain` 또는 `rdfs:range`로 직접 참조하면 안 된다.

다른 domain의 개념을 참조해야 할 경우 반드시 Core class 또는 Mediation Concept를 domain 또는 range로 사용해야 한다.
...
일부 cross-domain 관계는 단순히 core:Location 같은 추상 class만으로 충분하지 않다.

예를 들어 다음 관계는 복잡하다.

로봇 미션이 특정 작업구역, 특정 시간, 특정 허가 상태, 특정 위험 상태, 특정 작업자 존재 여부와 연결됨.  
센서 evidence가 특정 구역, 특정 장비 상태, 특정 policy 판단과 함께 action candidate를 지지함.  
Construction task와 robot mission이 같은 operational context에서 조율됨.

이 경우 direct reference 대신 Mediation Concept를 사용한다.

---

## **19.2 권장 Mediation Concept**

core-crosscutting에 다음 class를 둔다.
...
## **19.3 예시: Robot Mission과 Construction Zone 연결**
...
Robot Module은 Construction Module을 직접 물지 않는다.  
OperationalContext가 관계를 중재한다.

---

# **20\. Reasoning Wall and SHACL Validation**

## **20.1 Reasoning Wall 문제**

온톨로지가 커지고 mapping module이 많아지면 OWL Reasoner의 계산 비용이 급격히 증가할 수 있다.
...
이 모든 것을 runtime에서 매번 HermiT / Pellet으로 추론하면 시스템이 멈출 수 있다.

이를 Reasoning Wall이라고 부른다.

---

## **20.2 Offline / Build-time Reasoning Rule**

OWL Reasoner는 정적 일관성 검사용으로 사용한다.
...
Runtime에서는 전체 OWL reasoning을 최소화하고 SHACL을 주 방어선으로 사용한다.
...
MVP 단계부터 다음 항목은 SHACL로 검증해야 한다.
...
각 ontology module은 독립 namespace를 가져야 한다.
...
## **21.2 Versioned IRI 예시**

운영 환경에서는 versioned IRI를 사용할 수 있다.
...
class IRI는 안정적으로 유지한다.  
module version IRI는 release 관리에 사용한다.  
기존 class IRI를 함부로 변경하지 않는다.  
Breaking change는 새 version IRI 또는 새 module로 분리한다.
...
class IRI와 individual IRI를 구분해야 한다.
...
Construction Module은 core-upper와 core-crosscutting을 import할 수 있다.
...
Robot Module도 core를 import할 수 있다.
...
Core Upper가 Domain Module을 import하면 안 된다.
...
Robot Module이 Construction Module을 직접 import하는 것도 기본적으로 금지한다.
...
이 경우 Core abstraction 또는 Mediation Concept를 사용한다.

---

# **23\. Module Boundary Rule**

## **23.1 Core에 들어갈 수 있는 것**

다음 조건을 만족하면 Core에 들어갈 수 있다.

모든 domain에서 공통으로 쓰인다.  
특정 산업이나 장비에 종속되지 않는다.  
장기적으로 안정적이다.  
다른 module이 참조해야 하는 상위 개념이다.
...
## **23.2 Domain Module에 들어갈 것**

다음 조건을 만족하면 Domain Module에 둔다.

특정 산업, 현장, 장비, 업무에 특화되어 있다.  
Core 개념을 확장한다.  
다른 도메인에서는 사용되지 않을 수 있다.
...
## **23.3 Mapping Module에 들어갈 것**

다음 조건을 만족하면 Mapping Module에 둔다.

외부 표준과 내부 ontology를 연결한다.  
vendor schema와 내부 class를 연결한다.  
동일하거나 유사한 개념을 alignment한다.  
외부 schema 변화에 따른 adapter 역할을 수행한다.
...
## **23.4 Registry에 들어갈 것**

다음은 ontology module이 아니라 registry에서 관리한다.
...
모든 class와 property를 하나의 ontology 파일에 넣는 방식.
...
Core가 커지고 불안정해진다.  
다른 domain module이 오염된다.

---

## **24.3 Vendor Pollution**

Vendor-specific API나 tag를 ontology core에 직접 넣는 방식.
...
vendor 변경 시 core ontology가 흔들린다.

Vendor-specific 개념은 External Mapping Module 또는 Adapter Layer에 둔다.

---

## **24.4 AI Pollution**

AI output을 truth 또는 approved action으로 ontology에 직접 넣는 방식.
...
ObjectProperty의 domain 또는 range를 통해 타 domain module을 직접 참조하는 방식.
...
Ontology Module은 OntologyModuleSpecDTO로 관리한다.

## **25.1 OntologyModuleSpecDTO 필드**
...
LOW  
→ annotation, description, optional metadata 수준 변경

MEDIUM  
→ 새 class, 새 optional property, 새 mapping 추가

HIGH  
→ domain / range 변경, SHACL rule 강화, major mapping 변경

CRITICAL  
→ Core 변경, ObjectProperty boundary 변경, safety-critical class 변경, Action / State / Evidence 의미 변경

CRITICAL 변경은 반드시 다음 승인을 요구한다.
...
Evidence Module이나 Privacy 관련 변경의 경우 추가로 다음 승인이 필요하다.
...
모듈 간 참조는 CrossModuleReferenceSpecDTO로 관리한다.

## **26.1 CrossModuleReferenceSpecDTO 필드**
...
ObjectProperty 우회 오염을 막기 위해 별도 스펙을 둔다.

## **27.1 ObjectPropertyBoundarySpecDTO 필드**
...
## **27.2 boundary\_violation\_status 값**
...
External Mapping Module은 Semantic Adapter로 관리한다.

## **28.1 SemanticAdapterSpecDTO 필드**
...
## **28.3 mapping\_conflict\_resolution\_strategy 값**
...
Runtime validation layer는 ModuleValidationResultDTO를 생성해야 한다.

## **29.1 ModuleValidationResultDTO 필드**
...
실제 TTL, Python class, registry, mapping이 생성될 때 runtime validator가 반드시 검증해야 한다.
...
Ontology Module Boundary 검증은 CI/CD 파이프라인에서 자동 실행되어야 한다.
...
각 ontology module은 owner를 가져야 한다.
...
Module owner는 다음을 책임진다.

class 추가 승인  
property 추가 승인  
ObjectProperty domain / range 승인  
module import 승인  
breaking change 승인  
impact level 승인  
deprecation 승인  
external mapping 승인  
SHACL validation 통과 확인  
reasoner consistency check 통과 확인  
...
Ontology Module은 Semantic Versioning을 따른다.
...
MINOR는 backward-compatible change다.
...
annotation 추가  
external mapping 추가  
SHACL warning rule 추가  
description 보강  
새 mapping adapter 추가  
...
cardinality 강화  
상위 class 변경  
module dependency 방향 변경  
기존 mapping 제거  
ObjectProperty boundary rule 완화  
Core가 Domain을 참조하도록 변경

MAJOR 변경은 별도 migration plan과 downstream impact analysis를 요구한다.

---

# **34\. Deprecation Rule**

Ontology class, property, module은 바로 삭제하지 않는다.

권장 절차:

DEPRECATED 상태로 변경  
replacement\_class 또는 replacement\_property 지정  
dual-read 기간 운영  
mapping 유지  
...
LOW impact: 최소 14일  
MEDIUM impact: 최소 30일  
HIGH impact: 최소 60일  
CRITICAL impact: 최소 90일 또는 Safety Owner 승인 필요

---

# **35\. Governance and Validation Rule**

Ontology Module 변경은 governance 절차를 거쳐야 한다.

고위험 module 변경은 다음 검증을 통과해야 한다.
...
특히 다음 module은 stricter review가 필요하다.
...
Worker 자체는 Construction Module에 둔다.  
HumanAgent는 Core Upper Module에 둔다.

---

## **36.2 시나리오 2: GasSensor는 어디에 들어가는가?**
...
GasSensor class는 Industrial Module에 둔다.  
SensorObservationEvidence는 Evidence Module에 둔다.

---

## **36.3 시나리오 3: Robot Mission과 Construction Zone 연결**

문제:

RobotMission은 Robot Module에 있고, Zone은 Construction Module에 있다.  
둘을 직접 강하게 결합하면 module coupling이 커진다.
...
Robot Mission은 Construction Zone을 직접 class dependency로 물지 않는다.  
OperationalContext와 Location을 통해 연결한다.

---

## **36.4 시나리오 4: ObjectProperty 우회 오염 방지**

문제:

Robot Module에서 `robot:operatesIn`의 range를 `construction:WorkZone`으로 지정하고 싶다.
...
타 Domain class를 ObjectProperty의 range로 직접 물지 않는다.  
Core abstraction 또는 Mediation Concept를 사용한다.

---

## **36.5 시나리오 5: LLM이 생성한 ActionCandidate**

문제:

LLM이 ACTION\_EVACUATE\_ZONE을 제안했다.
...
AI output은 candidate까지만 가능하다.  
ApprovedAction은 Action / Policy / Safety Gate 경계를 통과해야 한다.

---

## **36.6 시나리오 6: IFC와 내부 Construction Ontology 연결**

문제:

IFC의 IfcWall과 내부 construction:Wall을 연결해야 한다.
...
IFC mapping은 Construction Module에 직접 섞지 않는다.  
External Mapping Module에서 관리한다.

---

## **36.7 시나리오 7: Vendor Robot API 변경**

문제:

Vendor Robot API v1에서 v2로 변경되었다.  
하지만 내부 robot ontology는 그대로 유지해야 한다.
...
내부 `robot:RobotMission`은 변경하지 않는다.  
Mapping Adapter만 교체한다.

---

## **36.8 시나리오 8: Evidence Model과 Evidence Module 연결**

문제:

Evidence Model에서 `clock_drift_estimate_ms`, `time_trust_level`, `trust_upgrade_status`, `privacy_lifecycle_status`가 정의되었다.  
이것이 Ontology Module Boundary에서 어디에 위치해야 하는가?
...
운영 필드는 EvidenceRecordDTO에서 관리한다.  
그 의미 class와 property는 Evidence Ontology Module에서 정의한다.  
Core는 Evidence라는 상위 개념만 가진다.

---

# **37\. MVP Ontology Module Set**

MVP에서는 다음 module부터 구현한다.
...
ObjectPropertyBoundarySpecDTO와 ObjectPropertyBoundaryValidator는 MVP 초기에 구현해야 한다.

이유:

Class boundary보다 property boundary에서 오염이 더 자주 발생한다.  
초기에 막지 않으면 나중에 refactoring 비용이 커진다.

---

# **40\. 최종 원칙**

Ontology Module Boundary는 플랫폼의 의미 경계선이다.

Core는 작고 안정적이어야 한다.  
Core는 core-upper와 core-crosscutting으로 분리한다.  
Domain Module은 Core를 확장해야 한다.  
Core는 Domain을 참조하면 안 된다.  
Vendor-specific 개념은 Core에 들어가면 안 된다.  
AI output은 truth나 ApprovedAction이 되면 안 된다.  
Registry는 운영 규칙을 관리하고, Ontology는 의미 구조를 정의한다.  
Evidence Module은 Evidence Model의 time trust, attested extraction, conflict resolution, privacy lifecycle을 의미적으로 수용해야 한다.  
External Mapping Module은 외부 표준과 내부 의미 체계를 연결한다.  
Mapping Module은 hot-swappable Semantic Adapter로 관리한다.  
Mapping conflict는 Mapping Layer에서 해결하고 내부 ontology를 변경하지 않는다.  
Class boundary뿐 아니라 ObjectProperty domain / range boundary도 검증해야 한다.  
복잡한 cross-domain 관계는 Mediation Concept로 중재한다.  
OWL Reasoner는 정적 일관성 검사용이고, Runtime 방어선은 SHACL이어야 한다.  
Module Boundary는 문서 규칙이 아니라 validator와 CI/CD pipeline이 강제해야 한다.  
Ontology 변경은 SemVer와 impact\_level로 관리해야 한다.
