...
# **LEDO 온톨로지 중심 레이어 아키텍처**

## **1\. 목적**

이 문서는 LEDO Ontology Core의 최종 레이어 아키텍처를 정의한다.

LEDO는 의미, 증거, 상태, 정책, 승인, 검증, 실행 요청, 피드백, 감사를 명확한 책임 경계로 분리하는 온톨로지 중심 Cyber-Physical AI 아키텍처다.
...
| Layer | 이름 | 핵심 책임 |
| ----- | ----- | ----- |
| 0 | Observability / Audit / Trace Layer | 이벤트, 증거, 판단, 승인, 실행 요청, 피드백, 실패, 온톨로지 변경, 운영 이력 추적 |
| 1 | Experience / Presentation Layer | 운영자, 감독자, 관리자, 안전팀, 경영진을 위한 사용자 화면 제공 |
| 2 | API Gateway Layer | 접근, 라우팅, 요청 검증, 실시간 통신, 외부 진입점 통제 |
| 3 | Governance / Policy / Security Layer | 권한, 정책, 승인, 보안, 컴플라이언스, 판단 권한 정의 |
| 4 | Core Ontology Kernel Layer | 객체, 관계, 속성, 사건, 행동, 제약, 워크플로우, 추론 계약의 형식적 의미 정의 |
| 5 | Knowledge & Semantic Memory Layer | 정적 지식, 문서, 그래프, 과거 기록, Evidence, materialized semantic knowledge 저장 |
| 6 | Real-Time World State Layer | 실시간 신호를 현재 운영 상태로 변환 |
| 7 | Distributed Domain Agent Layer | 도메인 상태를 해석하고 Alert, ActionCandidate, EscalationCase, DecisionCase 생성 |
| 8 | Decision Router / Escalation Layer | 위험도, 긴급도, 승인 필요성, 운영 영향, escalation 정책에 따라 판단 경로 분기 |
| 9 | Approved Action / Safety Gate Layer | 승인된 조치가 ExecutionRequest로 전환될 준비가 되었는지 검증 |
| 10 | Unified Cyber-Physical Core Layer | Action, Decision, Approval, ExecutionRequest, Feedback, Recovery, Audit의 공통 운영 생명주기 구조화 |
| 11 | Execution Request & External Control Integration Layer | 외부 제어 시스템에 high-level execution request 전달 |
| 12 | Physical World Layer | 실제 작업자, 로봇, 장비, 센서, PLC, SCADA, 현장, 구역, 외부 제어 시스템 표현 |
...
## **8\. Unified Cyber-Physical Core 위치**

Layer 10은 별도의 root-level 폴더를 필요로 하지 않는다.

Unified Cyber-Physical Core Layer는 `03_core_specifications/` 전체에 걸쳐 정의된다.
...
Layer 10은 다음 객체들의 공통 운영 생명주기를 연결한다.
...
Safety Gate는 승인된 조치의 실행 준비 상태를 검증한다.

Safety Gate는 승인 권한을 부여하지 않는다.

Approval은 권한을 부여한다.  
Safety Gate는 실행 준비 상태를 검증한다.

Safety Gate는 결정론적이며 fail-closed여야 한다.

Runtime hot path에서는 다음을 수행해서는 안 된다.
...
Runtime hot path는 materialized Safety Snapshot의 사전 계산된 검증 결과만 조회해야 한다.

---

## **10\. Execution Boundary 원칙**
...
Agent output은 후보이며 반드시 검증되어야 한다.
...
이 불변 조건을 위반하는 설계는 LEDO 아키텍처가 아니다.

---

## **13\. Final Architecture Statement**

LEDO는 책임 경계를 기준으로 계층화된다.

Ontology는 의미를 정의하고, Evidence는 판단 근거를 제공하며, World State는 현재 상태를 표현하고, Agent는 후보를 생성하며, Policy는 운영 허용 여부를 판단하고, Approval은 고위험 권한을 부여하며, Safety Gate는 실행 준비 상태를 검증하고, Unified Cyber-Physical Core는 운영 생명주기를 구조화하며, External Control Integration은 제한된 요청을 전달하고, External System은 실제 물리 실행을 수행한다.
