...
# **identity\_registry 설계 보고서**

## **1\. 개요**

`identity_registry`는 LEDO Ontology-Centric Cyber-Physical System에서 사용되는 모든 Human Identity, Agent Identity, Service Identity, System Identity, External System Identity, Role, Attribute, Certification, Site Assignment, Zone Scope, Shift Status, Delegation Status, Identity Trust Level을 정의하고 통제하는 핵심 레지스트리이다.

이 모듈의 목적은 시스템 내부에서 발생하는 Action 제안, Decision 생성, Approval 수행, Safety Gate 검증, External System 요청, Audit Trace가 모두 “누가 수행했는가”를 명확히 추적할 수 있도록 하는 것이다.

`identity_registry`는 단순한 사용자 목록이 아니다.

이 레지스트리는 다음을 정의하는 **신원·역할·속성·범위·위임·추적성 운영 계약 레지스트리**이다.

이 주체는 누구인가?  
사람인가, Agent인가, Service인가, External System인가?  
이 주체는 어떤 role을 가지고 있는가?  
이 주체는 어떤 site와 zone에서 유효한가?  
이 주체는 현재 shift/on-duty 상태인가?  
이 주체는 어떤 certification을 가지고 있는가?  
이 주체는 어떤 approval authority를 가질 수 있는가?  
이 주체는 어떤 ActionCandidate를 제안할 수 있는가?  
이 주체는 어떤 system access를 가질 수 있는가?  
이 주체의 권한은 위임된 것인가, 원래 권한인가?  
이 주체의 행동은 audit 가능한가?

즉, `identity_registry`는 LEDO 시스템에서 “누가 어떤 자격으로 이 행위를 했는가?”를 결정론적으로 통제하는 핵심 레지스트리이다.
...
Identity는 권한 그 자체가 아니다.

Identity는 Approval이 아니다.

Identity는 Policy가 아니다.

Identity는 Safety Gate가 아니다.

Identity는 Physical Command가 아니다.

Identity의 기본 의미는 다음과 같다.

이 행위를 수행한 주체는 누구인가?  
이 주체는 사람인가, Agent인가, Service인가?  
이 주체는 어떤 role과 attribute를 가지고 있는가?  
이 role은 어느 site, zone, shift 범위에서 유효한가?  
이 주체는 승인·검토·제안·실행 요청을 할 자격이 있는가?
...
`identity_registry`는 “누구인가”와 “어떤 속성을 가지고 있는가”를 제공한다.  
최종 허용 여부는 `policy_registry`, `approval_registry`, `access_control_registry`, `safety_gate`가 함께 판단한다.

---

## **3\. LEDO 아키텍처 내 위치**

`identity_registry`는 Governance / Policy / Security Layer에 위치하지만, LEDO 전체 lifecycle을 가로지르는 cross-cutting registry이다.

Human / Agent / Service / External System  
        ↓  
identity\_registry validation  
        ↓  
Role / Attribute / Scope / Certification 확인  
        ↓  
Policy Evaluation  
        ↓  
Decision / Approval / Execution / Audit

`identity_registry`는 다음 흐름에서 반드시 사용된다.

Agent Output  
    → Agent Identity 확인

ActionCandidate  
    → proposed\_by identity 확인

DecisionCase  
    → decision actor identity 확인

ApprovalRequest  
    → approver identity / role / scope / certification 확인

SafetyGateResult  
    → validator identity 확인

ExecutionRequest  
    → service identity / adapter identity 확인

ExternalControlRequest  
    → external system identity 확인

AuditRecord  
    → 모든 actor identity 기록

---

## **4\. 목적**

`identity_registry`의 목적은 다음과 같다.

1. 등록되지 않은 Identity 사용 방지  
2. Human / Agent / Service / External System Identity 구분  
3. Identity별 role 정의  
4. Identity별 attribute 정의  
5. Identity별 site / zone / operation scope 정의  
6. Identity별 shift / duty status 정의  
7. Identity별 certification / training 상태 정의  
8. Identity trust level 정의  
9. Approval authority 검증 지원  
10. Agent authority 검증 지원  
11. Service-to-service request 검증 지원  
12. External system trust 검증 지원  
13. Delegation 및 temporary authority 관리  
14. PII / sensitivity rule 관리  
15. Audit trace에서 actor를 재현 가능하게 관리  
16. Identity lifecycle 및 revocation 관리
...
`Identity`는 LEDO 시스템에서 행위 주체를 나타내는 고유 신원이다.
...
Identity는 “누구인가?”를 표현한다.

---

### **5.2 Identity Type**

`Identity Type`은 주체의 종류이다.
...
각 Identity Type은 다른 권한 경계와 검증 기준을 가진다.

---

### **5.3 Role**

`Role`은 identity가 수행할 수 있는 조직적 또는 시스템적 역할이다.
...
`Attribute`는 identity의 상태나 조건을 나타내는 속성이다.
...
Attribute는 ABAC, Approval, Policy Evaluation에 사용된다.

---

### **5.5 Scope**

`Scope`는 identity의 역할과 권한이 유효한 범위이다.
...
`Certification`은 특정 위험 작업, 승인, 로봇 운용, 안전 판단에 필요한 자격이다.
...
Safety-critical approval에는 certification 확인이 필수이다.

---

### **5.7 Delegation**

`Delegation`은 한 identity의 권한이 다른 identity에게 제한적으로 위임되는 것이다.

예시:

Safety Supervisor가 부재 중일 때,  
Site Safety Manager가 특정 site와 shift 범위에서 safety approval 권한을 위임받는다.

Delegation은 반드시 다음을 가져야 한다.
...
`identity_registry`는 다음 항목을 통제한다.
...
`identity_registry`는 다음을 정의하지 않는다.
...
5. HR 시스템 전체 데이터  
6. 모든 근로자 개인정보 원본  
7. policy pass/fail logic 전체  
8. approval rule 전체  
9. Safety Gate 최종 판정  
10. low-level physical command  
11. adapter 실행 로직  
12. external system 내부 계정 체계 전체  
13. 모든 access control rule 전체  
14. 모든 session management 구현
...
`identity_registry`는 실제 비밀번호나 token을 저장하지 않는다.  
필요한 경우 외부 Identity Provider, IAM, SSO, OIDC, Vault와 reference로 연결한다.

---

## **8\. Identity Type 모델**

권장 Identity Type은 다음과 같다.
...
Human Identity는 PII, role, certification, shift, site assignment를 고려해야 한다.

---

### **8.2 AGENT\_IDENTITY**

LEDO 내부 또는 edge에서 동작하는 Agent를 나타낸다.
...
Agent Identity는 `agent_vocabulary_registry`와 연결되어야 한다.

Agent는 사람이 아니며, Approval Authority가 될 수 없다.

---

### **8.3 SERVICE\_IDENTITY**

LEDO 내부 backend service 또는 runtime component를 나타낸다.
...
Service Identity는 service-to-service authorization, audit, mTLS, signed request에 사용된다.

---

### **8.4 ADAPTER\_IDENTITY**

LEDO 내부 external integration adapter를 나타낸다.
...
Adapter Identity는 `adapter_registry`와 연결된다.

---

### **8.5 EXTERNAL\_SYSTEM\_IDENTITY**

LEDO 외부 시스템의 identity이다.
...
External System Identity는 `external_system_registry`와 연결된다.

---

### **8.6 DEVICE\_IDENTITY**

센서, gateway, camera, IoT device, PLC gateway 등 장치 identity이다.
...
Device Identity는 event producer, evidence source 검증에 사용된다.

---

### **8.7 ORGANIZATION\_IDENTITY**

회사, 협력업체, vendor, 외부 운영 주체를 나타낸다.
...
Organization Identity는 ownership, vendor responsibility, liability, contract scope에 연결된다.

---

### **8.8 TEMPORARY\_IDENTITY**

임시 접근 권한 또는 제한된 시간 동안 유효한 identity이다.
...
Temporary Identity는 반드시 만료시간과 scope를 가져야 한다.
...
주로 Agent 또는 supervisor workflow에서 사용된다.

---

### **9.4 review**

DecisionCase, EvidenceBundle, RiskSignal 등을 검토할 수 있다.

---

### **9.5 approve**

ApprovalDecision을 수행할 수 있다.

단, approval\_registry의 rule, scope, certification 조건을 만족해야 한다.

---

### **9.6 administer**

시스템 설정, registry 관리, 사용자 관리 등 관리 기능을 수행할 수 있다.

---

### **9.7 service\_execute**

내부 service가 시스템 lifecycle object를 처리할 수 있다.
...
외부 시스템으로서 event, feedback, execution result를 제공할 수 있다.

---

## **10\. Registry Entry Schema**

각 Identity Registry entry는 다음 구조를 따른다.
...
## **11\. Registry Entry 예시: Safety Supervisor**

identity\_id: human:safety\_supervisor\_001  
canonical\_name: safety\_supervisor\_001  
display\_name: Safety Supervisor 001  
description: Site A의 safety-critical approval과 safety review를 수행할 수 있는 안전관리자 identity이다.  
semantic\_iri: ledo:SafetySupervisorIdentity001
...
## **12\. Registry Entry 예시: Robot Dispatch Agent Identity**

identity\_id: agent:robot\_dispatch\_agent\_site\_A  
canonical\_name: robot\_dispatch\_agent\_site\_A  
display\_name: Robot Dispatch Agent \- Site A  
description: Site A에서 robot dispatch 관련 ActionCandidate와 EvidenceBundle을 생성할 수 있는 Agent identity이다.  
semantic\_iri: ledo:RobotDispatchAgentIdentitySiteA
...
## **13\. Registry Entry 예시: Execution Dispatcher Service Identity**

identity\_id: service:execution\_dispatcher  
canonical\_name: execution\_dispatcher  
display\_name: Execution Dispatcher Service  
description: Safety Gate를 통과한 ExecutionRequest를 adapter로 전달하는 내부 service identity이다.  
semantic\_iri: ledo:ExecutionDispatcherServiceIdentity
...
## **14\. Registry Entry 예시: External System Identity**

identity\_id: external\_system:robot\_fleet\_manager\_site\_A  
canonical\_name: robot\_fleet\_manager\_site\_A  
display\_name: Robot Fleet Manager Identity \- Site A  
description: Site A robot fleet manager external system의 identity이다.  
semantic\_iri: ledo:RobotFleetManagerIdentitySiteA
...
중요한 점은 Identity가 active라고 해서 모든 권한을 갖는 것은 아니라는 것이다.

Identity가 active 상태여야 한다.  
필요한 role을 가져야 한다.  
필요한 attribute를 가져야 한다.  
필요한 certification을 가져야 한다.  
site / zone / shift scope가 일치해야 한다.  
해당 action, approval, service, external request boundary를 만족해야 한다.

---

## **16\. Validation Rules**

Identity Entry는 다음 조건을 만족할 때만 유효하다.

1. `identity_id`가 registry에 존재해야 한다.  
2. status가 `active`이어야 한다.  
3. identity type이 선언되어야 한다.  
4. role reference가 선언되어야 한다.  
5. authority level이 선언되어야 한다.  
6. trust level이 선언되어야 한다.  
7. site / zone / system scope가 선언되어야 한다.  
8. valid\_from / valid\_until이 선언되어야 한다.  
9. PII classification이 선언되어야 한다.  
10. sensitivity level이 선언되어야 한다.  
11. decision / approval / execution / safety boundary가 선언되어야 한다.  
12. audit event reference가 선언되어야 한다.  
13. owner module이 선언되어야 한다.  
14. version이 유효해야 한다.  
15. deprecated 상태라면 migration metadata가 있어야 한다.

하나라도 누락되면 해당 Identity는 operational lifecycle에 사용되면 안 된다.

---

## **17\. Runtime Identity Validation**

Runtime에서 identity를 사용할 때는 다음 검증이 필요하다.

Identity가 registry에 존재하는가?  
Identity가 active 상태인가?  
Identity가 만료되지 않았는가?  
Identity가 revoked 또는 suspended 상태가 아닌가?  
필요한 role을 가지고 있는가?  
필요한 attribute를 가지고 있는가?  
필요한 certification을 가지고 있는가?  
site scope가 일치하는가?  
zone scope가 일치하는가?  
shift scope가 일치하는가?  
MFA가 필요한 경우 verified 상태인가?  
delegation이 필요한 경우 유효한 delegation인가?  
boundary를 위반하지 않는가?  
audit trace가 생성되는가?

이 조건을 만족하지 못하면 해당 identity는 요청을 수행할 수 없다.

---

## **18\. Role Rule**

Role은 반드시 registry 기반으로 관리되어야 한다.
...
Role은 권한 판단의 입력값이지만, 최종 허용 여부는 Policy와 Scope로 결정된다.
...
safety\_supervisor role이 있어도  
site\_A scope가 없으면 site\_A STOP\_WORK approval을 할 수 없다.

---

## **19\. Attribute Rule**

Attribute는 identity의 현재 상태를 표현한다.
...
Attribute는 자주 변할 수 있다.  
따라서 장기 identity record와 runtime attribute snapshot을 분리할 수 있다.

Identity Registry:  
    identity의 기본 속성 및 허용 속성 정의

Runtime Attribute Store:  
    현재 on\_shift, active\_session, mfa\_verified 등 동적 상태 관리

---

## **20\. Scope Rule**

Scope는 identity가 유효한 범위를 제한한다.

Scope는 반드시 다음에 적용되어야 한다.
...
Safety-critical action이나 approval에는 certification 검증이 필요하다.

예시:

STOP\_WORK approval:  
    construction\_safety\_supervisor\_certification 필요

DISPATCH\_ROBOT approval:  
    robot\_operation\_supervisor\_training 필요

Electrical isolation approval:  
    electrical\_safety\_certification 필요

Certification은 만료될 수 있다.
...
Delegation은 제한적으로 허용되어야 한다.

Delegation이 유효하려면 다음 조건이 필요하다.

1. delegator identity가 active여야 한다.  
2. delegate identity가 active여야 한다.  
3. delegator가 위임 가능한 authority를 가지고 있어야 한다.  
4. delegate가 필요한 최소 role 또는 certification을 가지고 있어야 한다.  
5. delegation scope가 명확해야 한다.  
6. delegation valid\_until이 있어야 한다.  
7. delegation policy가 존재해야 한다.  
8. audit record가 생성되어야 한다.
...
## **23\. PII 및 Privacy Rule**

Identity Registry는 민감한 개인정보를 과도하게 저장하면 안 된다.

권장 원칙:

최소한의 identity reference만 저장한다.  
실제 개인정보는 HR system 또는 IAM provider에 둔다.  
LEDO에는 operational identity metadata만 둔다.  
PII는 masking, encryption, access control, audit가 필요하다.
...
device identity linked to worker:  
    pii\_classification: indirect 또는 direct

---

## **24\. Relationship to Access Control Registry**

`identity_registry`는 “누구인가”와 “어떤 속성을 가지는가”를 정의한다.

`access_control_registry`는 “무엇에 접근할 수 있는가”를 정의한다.

identity\_registry:  
    safety\_supervisor\_001은 safety\_supervisor role과 site\_A scope를 가진다.

access\_control\_registry:  
    safety\_supervisor role은 safety dashboard와 approval UI에 접근할 수 있다.

Identity는 Access Control의 입력이다.

---

## **25\. Relationship to Policy Registry**

`policy_registry`는 정책 판단을 정의한다.

`identity_registry`는 policy evaluation에 필요한 identity claims를 제공한다.

identity\_registry:  
    이 사용자는 site\_A safety\_supervisor이고 on\_shift 상태이다.

policy\_registry:  
    STOP\_WORK approval은 site scope와 on\_shift safety supervisor를 요구한다.

Identity는 policy를 대체하지 않는다.

---

## **26\. Relationship to Approval Registry**

`approval_registry`는 어떤 approval에 어떤 role, certification, scope가 필요한지 정의한다.

`identity_registry`는 현재 approver가 그 조건을 만족하는지 검증한다.

approval\_registry:  
    STOP\_WORK에는 safety\_supervisor\_approval이 필요하다.

identity\_registry:  
    이 approver는 safety\_supervisor role, site\_A scope, certification을 가지고 있는가?
...
`agent_vocabulary_registry`는 어떤 Agent Type이 어떤 output을 만들 수 있는지 정의한다.

`identity_registry`는 실제 runtime Agent Instance가 등록된 신원인지 검증한다.

agent\_vocabulary\_registry:  
    ROBOT\_DISPATCH\_AGENT는 DISPATCH\_ROBOT ActionCandidate를 제안할 수 있다.

identity\_registry:  
    agent:robot\_dispatch\_agent\_site\_A는 실제 등록된 active agent identity인가?

Agent Identity가 없으면 Agent Output은 신뢰할 수 없다.

---

## **28\. Relationship to Event Registry**

Event는 producer identity를 가져야 한다.

event\_registry:  
    WorkerLocationUpdated event는 registered sensor gateway가 produce할 수 있다.

identity\_registry:  
    producer device 또는 service가 실제 등록된 identity인가?

등록되지 않은 producer가 만든 event는 reject 또는 quarantine되어야 한다.

---

## **29\. Relationship to Evidence Registry**

Evidence는 producer identity와 source identity를 가져야 한다.

evidence\_registry:  
    worker\_location\_snapshot은 worker\_tracking\_gateway가 만들 수 있다.

identity\_registry:  
    해당 gateway identity가 active이고 trusted 상태인가?

Identity가 불명확한 evidence는 judgment에 사용하면 안 된다.

---

## **30\. Relationship to Adapter Registry**

Adapter는 identity를 가져야 한다.

adapter\_registry:  
    robot\_fleet\_adapter\_site\_A는 robot mission request를 처리할 수 있다.

identity\_registry:  
    adapter:robot\_fleet\_adapter\_site\_A는 등록된 active adapter identity인가?

Adapter Identity가 invalid이면 External System 요청을 보내면 안 된다.

---

## **31\. Relationship to External System Registry**

External System도 identity를 가져야 한다.

external\_system\_registry:  
    robot\_fleet\_manager\_site\_A는 registered external system이다.

identity\_registry:  
    external\_system:robot\_fleet\_manager\_site\_A는 trusted external system identity인가?

External System Identity는 mTLS, certificate, signed feedback validation에 사용될 수 있다.

---

## **32\. Relationship to Audit Registry**

모든 중요한 행위는 actor identity를 가져야 한다.
...
모든 중요한 Identity Type과 Role은 semantic IRI를 가져야 한다.
...
Ontology는 Identity와 Role의 의미론적 기반을 제공한다.

Identity Registry는 이를 운영 시스템에서 version, status, scope, certification, delegation, audit rule로 관리한다.

---

## **34\. Versioning 및 Migration**

Identity Entry는 반드시 versioning되어야 한다.

다음 항목 중 하나라도 변경되면 version 변경이 필요하다.

1. identity type 변경  
2. role reference 변경  
3. authority level 변경  
4. trust level 변경  
5. site / zone / operation scope 변경  
6. certification requirement 변경  
7. delegation rule 변경  
8. access control reference 변경  
9. policy reference 변경  
10. approval authority reference 변경  
11. decision / approval / execution / safety boundary 변경  
12. PII classification 변경
...
권한이 취소된 identity이다.  
새로운 lifecycle action에 사용되면 안 된다.

---

### **34.3 expired**

valid\_until이 지난 identity이다.

---

### **34.4 blocked**

보안, 안전, 감사 사유로 차단된 identity이다.

Blocked Identity는 어떤 operational lifecycle에도 사용하면 안 된다.

---

## **35\. Implementation Use**

`identity_registry`는 다음을 생성하거나 검증하는 데 사용된다.
...
Implementation은 등록되지 않은 identity를 operational actor로 사용하면 안 된다.
...
1\. 등록되지 않은 Identity 거부  
2\. inactive Identity 거부  
3\. suspended Identity 거부  
4\. revoked Identity 거부  
5\. expired Identity 거부  
6\. blocked Identity 거부  
7\. required role 누락 거부  
8\. required attribute 누락 거부  
9\. required certification 누락 거부  
10\. site scope 불일치 거부  
11\. zone scope 불일치 거부  
12\. shift scope 불일치 거부  
13\. MFA required인데 verified가 아닌 경우 거부  
14\. delegation 만료 시 거부  
15\. delegation scope 불일치 거부  
16\. Agent Identity가 ApprovalDecision 생성 시도 시 거부  
17\. Service Identity가 승인 authority로 사용되는 경우 거부  
18\. External System Identity가 내부 Approval Authority로 사용되는 경우 거부  
19\. Identity 없는 Event Producer 거부  
20\. Identity 없는 Evidence Producer 거부  
21\. AuditRecord에 actor\_identity\_id 누락 시 거부  
22\. Identity migration rule 검증
...
유효한 Identity가 없으면,  
ActionCandidate도 신뢰할 수 없다.

유효한 Identity가 없으면,  
DecisionCase도 신뢰할 수 없다.

유효한 Identity가 없으면,  
ApprovalDecision도 신뢰할 수 없다.

유효한 Identity가 없으면,  
ExecutionRequest도 신뢰할 수 없다.

Role만으로는 충분하지 않다.

Role \+ Attribute \+ Scope \+ Certification \+ Policy가 필요하다.
...
Identity는 PhysicalCommand가 아니다.

`identity_registry`는 LEDO 시스템에서 모든 행위 주체의 신원, 역할, 속성, 범위, 자격, 위임, 신뢰 수준을 통제하는 핵심 결정론적 레지스트리이다.

이 모듈은 “누가 어떤 자격으로 이 행위를 했는가?”를 명확히 추적하고, 승인·판단·실행·감사 흐름에서 신원 없는 행위가 발생하지 않도록 보장한다.

핵심 정의는 다음과 같다.

Identity Registry  
\= 사용자 목록이 아니라,  
LEDO에서 행동하는 모든 Human, Agent, Service, Adapter,  
External System, Device의 신원, 역할, 속성, 범위,  
자격, 위임, 신뢰 수준, audit trace를 통제하는  
신원 운영 계약 레지스트리
